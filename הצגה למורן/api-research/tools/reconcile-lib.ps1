# research-swarm reconciliation. Dot-source AFTER swarm-lib.ps1 (reuses nothing from it, but they pair).
...
function Read-Jl { param([string]$Path) if (-not (Test-Path $Path)) { return @() }
...
$script:Rules = @{
  ...
  'recency' = {
    param($g)
    $dated = @($g | Where-Object { $_.asof } | ForEach-Object { $_ | Add-Member -NotePropertyName _d -NotePropertyValue ([datetime]::Parse($_.asof)) -PassThru -Force } 2>$null)
...
function Resolve-Findings {
  param(
    [Parameter(Mandatory)][string]$Findings,[string]$Reasons='reasons.jsonl',
